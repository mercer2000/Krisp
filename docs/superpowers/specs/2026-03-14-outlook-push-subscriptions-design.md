...
**Type changes in `lib/graph/subscriptions.ts`:** The `GraphSubscriptionInsert` interface must make `credentialId` optional (`credentialId?: string | null`) and add `outlookOauthTokenId?: string | null`.
...
1. **Capture the token row ID** from `upsertOutlookTokens()` (it already returns the full row via `RETURNING *`; the current callback discards the return value)
2. Get the user's access token from the just-stored token response
3. Generate a `clientState` (random UUID for webhook verification)
4. Build the resource path: `me/mailFolders/inbox/messages` (delegated tokens must use `/me/`, not `/users/{email}/` which requires app-only permissions)
5. Build the notification URL: `${origin}/api/webhooks/email/graph/${userId}`
6. POST to `https://graph.microsoft.com/v1.0/subscriptions` with:
   - `changeType: "created"`
   - `resource`: the path above
   - `expirationDateTime`: now + 4230 minutes (~3 days, the Microsoft-imposed maximum for mail subscriptions)
   - `notificationUrl`: the webhook URL
   - `clientState`: the generated UUID
7. Store the subscription in `graphSubscriptions` with `outlookOauthTokenId` set to the captured token row ID and `credentialId` null
8. If subscription creation fails, log the error but still redirect to settings with `?connected=true` — the account is connected, push just isn't active yet. The catch-up cron will handle email sync in the meantime.
...
**Expand the `pendingFetches` type** to support both credential paths:
...
- If `credentialId` is present: existing path — use `getGraphCredentialsByIdUnsafe()` + `getGraphAccessTokenFromCreds()` (app-only token), then `fetchGraphMessage(userMailbox, messageId, token)` with the mailbox extracted from the resource path
- If `outlookOauthTokenId` is present: new path — use `getValidOutlookAccessToken(outlookOauthTokenId, tenantId)` (delegated token), then use a new `fetchGraphMessageMe(messageId, token)` function that calls `/me/messages/{id}` instead of `/users/{mailbox}/messages/{id}`. This follows the existing pattern in `lib/graph/messages.ts` where `deleteGraphMessageMe()` already exists for delegated-token operations. Skip `extractUserFromResource()` entirely for this path.
...
New `GET` route at `app/api/cron/graph-subscription-renewal/route.ts` (GET handler, consistent with all existing crons):
...
New `GET` route at `app/api/cron/outlook-catch-up/route.ts` (GET handler, consistent with all existing crons):
...
1. **API**: `GET /api/integrations/status` (already exists) — add a separate query or post-process the existing Outlook query to detect the "needs reconnection" state: rows where `active = false AND refresh_token IS NOT NULL`. Return this as a distinct `outlookNeedsReconnection: boolean` field in the response.
...
When a user intentionally disconnects their Outlook account (via settings UI), the disconnection flow should also deactivate any associated Graph subscriptions. Call `DELETE https://graph.microsoft.com/v1.0/subscriptions/{subscriptionId}` to clean up on Microsoft's side, then `deactivateSubscription()` locally. The `ON DELETE SET NULL` FK handles the case where the `outlook_oauth_tokens` row is deleted, but explicit disconnection typically just deactivates — so the cleanup must be done explicitly.
...
| `lib/graph/subscriptions.ts` | Make `credentialId` optional, add optional `outlookOauthTokenId` to `GraphSubscriptionInsert` |
| `lib/graph/messages.ts` | Add `fetchGraphMessageMe()` for delegated-token `/me/messages/{id}` calls |
| `app/api/outlook/oauth/callback/route.ts` | Capture token row ID, create Graph subscription after storing tokens |
| `app/api/webhooks/email/graph/[tenantId]/route.ts` | Expand `pendingFetches` type, add delegated-token branch in `after()` block |
| `app/api/cron/graph-subscription-renewal/route.ts` | New GET — hourly renewal cron |
| `app/api/cron/outlook-catch-up/route.ts` | New GET — hourly catch-up sync for stale accounts |
| `app/api/integrations/status/route.ts` | Add `outlookNeedsReconnection` flag to response |
| `components/ui/AppShell.tsx` | Render reconnection banner when flag is set |

**Vercel Cron configuration:** Add the two new cron schedules to the Vercel dashboard (or `vercel.json` if created):
- `/api/cron/graph-subscription-renewal` — `0 * * * *` (every hour)
- `/api/cron/outlook-catch-up` — `30 * * * *` (every hour at :30, offset from renewal)
